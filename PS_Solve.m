...
%--------------------------------------------------------------------------
function [t,X,U,f,p,ef] = PS_Solve(p)
    % default options
    p = PS_DefaultOpts(p);
    ...
    options = optimoptions('fmincon','Display','Iter',...
        'Algorithm','sqp','MaxFunEvals',Inf,...
         'StepTolerance', 100*eps,'TolConSQP',10^-6);
    %'OptimalityTolerance', Inf
	% additional options
    ...
    tic
    % solve the problem
    [x,f,exitflag] = fmincon(@(x) PS_Objective(x,p),x0,[],[],[],[],xL,xU,...
        @(x) PS_Nonlincon(x,p),options);
       % [x,f,exitflag] = fmincon(@(x) 0 ,x0,[],[],[],[],xL,xU,...
       % @(x) PS_Nonlincon(x,p),options);
