function ddmr_plot(t,X,U,f,p)
    ...
    for i = 1:length(p.Narray)
        ...
        interpU2{2,i} = LagrangeInter(p.t{i}',U(p.cumN(i)+1:p.cumN(i+1),2)',tarray1{i});
    end
    ...
    yc = [0 0 0];
    thetac = [0.3 0.3 0.3];
    u1c = [0.8500 0.3250 0.0980];
    u2c = [0.8400 0.3450 0.0940];
    lc = [76,175,80]/255;
    % defaults
    ...
    hL = legend('$x_{\mathrm{interp}}$','$y_{\mathrm{interp}}$',...
        '$theta_{\mathrm{interp}}$','$u1_{\mathrm{interp}}$','$u2_{\mathrm{interp}}$');
